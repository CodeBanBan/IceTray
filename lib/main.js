...
function _parseBody (ObjectSchema, rawBody) {
  ...
  for (let key in ObjectSchema) {
    if (_isNestedSchema(ObjectSchema, key)) {
      data[key] = _processNestedSchema(ObjectSchema, rawBody, key)
      continue
    }
...
function _processNestedSchema (ObjectSchema, rawBody, key) {
  ...
  if (Array.isArray(nestedSchema)) {
    ...
    if (typeof tempType === 'function') {
      // Case: Array of value
      for (let i = 0; i < nestedRawData.length; ++i) {
        const item = tempType(nestedRawData[i])
        result.push(item)
      }
    } else {
      // Case: Array of Object
      for (let i = 0; i < nestedRawData.length; ++i) {
        const item = _parseBody(tempType, nestedRawData[i])
        result.push(item)
      }
    }
  } else {
